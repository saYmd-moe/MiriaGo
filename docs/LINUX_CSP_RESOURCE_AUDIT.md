...
| MapLibre JS/CSS（当前过渡） | `unpkg.com` | `web/index.html` | 当前临时允许；M3 vendor 后必须删除该源 |
...
3. **M3 合并后**：M3 将 MapLibre JS/CSS 放入 `web/vendor/maplibre-gl/`。合并时从 `script-src`、`style-src` 和 `MiriaGoCsp.*Sources` 删除 `https://unpkg.com`，并保留 `'self'`；不需要 CDN 可用性测试或网络依赖。
