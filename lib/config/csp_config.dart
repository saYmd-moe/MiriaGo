...
/// Host the desktop Shell scripts/styles load the MapLibre engine from.
///
/// This is a *transitional* entry: milestone M3 vendors MapLibre GL JS/CSS
/// under `web/vendor/maplibre-gl/` (same-origin), at which point this host
/// should be dropped from [MiriaGoCsp.scriptSources] and
/// [MiriaGoCsp.styleSources].  Tests must not require this CDN to exist.
const transitionalMapEngineCdnHost = 'https://unpkg.com';

/// Builds the MiriaGo desktop CSP.
///
/// Design intent ("minimal usable policy"):
/// - Scripting surfaces are locked down: only same-origin scripts (plus the
///   transitional MapLibre CDN and CanvasKit WASM). No arbitrary remote
///   scripts, no inline script execution, no objects, no frames.
...
abstract final class MiriaGoCsp {
  ...
  static const List<String> scriptSources = <String>[
    ...
    transitionalMapEngineCdnHost,
  ];

  /// Entries for `style-src`. Flutter injects inline styles, and until M3
  /// vendors the MapLibre stylesheet we still load it from the CDN.
